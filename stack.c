...
struct stack *stack_new_array_at(struct stack *stk, size_t n, void **restrict elements)
{
	stack_new_prealloc_at(stk, n);
...
struct stack *stack_dup_at(struct stack *stk, struct stack *restrict orig)
{
	if (stk == NULL) return NULL;

	stack_new_prealloc_at(stk, orig->maxn);
