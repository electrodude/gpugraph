#ifndef OBJSTORE_STACK_H
#define OBJSTORE_STACK_H
...
#define stack_new_prealloc(maxn) stack_new_prealloc_at(stack_alloc_raw(), maxn)
struct stack *stack_new_prealloc_at(struct stack *stk, size_t maxn);

// Create a new stack
#define stack_new() stack_new_at(stack_alloc_raw())
#define stack_new_at(stk) stack_new_prealloc_at(stk, 16)

// Create a new stack and fill it with the given data
#define stack_new_v(n, ...) stack_new_v_at(stack_alloc_raw(), n, __VA_ARGS__)
struct stack *stack_new_v_at(struct stack *stk, size_t n, ...);

// Create a new stack and fill it with the given data
#define stack_new_array(n, elements) stack_new_array_at(stack_alloc_raw(), n, elements)
struct stack *stack_new_array_at(struct stack *stk, size_t n, void **elements);

// Clone a stack
#define stack_dup(orig) stack_dup_at(stack_alloc_raw(), orig)
struct stack *stack_dup_at(struct stack *stk, struct stack *orig);
